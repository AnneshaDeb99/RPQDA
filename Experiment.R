...
Experiment <- function(scheme, method, p_all, iter, n1 = 100, n2 = 100, m1 = 200, m2 = 200, file_name) {
  ...
  for (k in 1:length(p_all)) {
    ...
    set.seed(123)
    param <- Scheme(type = scheme, p)
  
  for (j in 1:iter) {
    keep <- c("Scheme", "k","p","param","j","Bayes","Experiment","generateData_Bayes","i","m2","m1","n1","n2","scheme", "path", "Ti", "file_name", "iter","method","MP","p_all","RPEnsemble")
