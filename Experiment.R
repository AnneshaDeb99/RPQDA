...
Experiment <- function(model, method, p_all, iter, n1=100, n2=100, m1=200, m2=200, file_name) {
  ...
  dimnames(MP)[[1]] <- c("Bayes","HDC", "AoYa", "RPE-CS","DA-QDA","IIS","RPE-SN","RPE-TP")
  dimnames(MP)[[3]] <- as.factor(p_all)
  dimnames(Ti)[[1]] <- c("Bayes","HDC", "AoYa", "RPE-CS","DA-QDA","IIS","RPE-SN","RPE-TP")
  ...
  for (j in 1:iter) {
    keep <- c("Model", "k","p","param","j","Bayes","Experiment","generateData_Bayes","i","IIS_SQDA","m2","m1","n1","n2","model","path","RPBag","sQDA","Ti","AoYa","file_name","HDClass","iter","method","MP","p_all","RPEnsemble")
    rm(list = setdiff(ls(), keep))
    source('HDDA.R')
    source('IIS-SQDA.R')
    source('RPE-CS.R')
    source('RPE-Our method.R')
    #source('Aoshima_Yata.R')
    source('AoYa.R')
    source('DA-QDA.R')
    source('generateData_Bayes.R')
    #source('AoYa.binary.R')
    source('RPE-CS.binary.R')
    
    #source('Bayes.R')
    
    set.seed(1000+j)

      dataX <- generateData_Bayes(param$mu1, param$mu2, param$sigma1, param$sigma2, param$Omega1, param$Omega2, param$logdet, p, n1, n2, m1, m2)
      bayes <- dataX$res
      cat("Starting iteration =", j, ", p =", p, "...\n")

      
      X_train <- rbind(dataX$X0[1:n1,],dataX$X1[1:n2,])
      y_train <- c(rep(0,n1),rep(1,n2))
      X_test <- rbind(dataX$X0[(n1+1):(n1+m1),],dataX$X1[(n2+1):(n2+m2),])
      y_test <- c(rep(0,m1),rep(1,m2))
      ...
      # HDClassif
      if (method %in% c('HDClassif', 'all')) {
        start_hd <- Sys.time()
        output <- HDClass(X_train, y_train, X_test)
      ...
      if (method %in% c('AoYa', 'all')) {
        start_ay <- Sys.time()
        output <- AoYa.binary(X_train,y_train,X_test)
      ...
      if (method %in% c('RPE-CS', 'all')) {
        start_bag <- Sys.time()
        output <- RPBag.binary(X_train,y_train,X_test, d=5, B1 = 500, B2 = 50)
        MP[4,j,k] <- mean(y_test!=(output-1))
        end_bag <- Sys.time()
      ...
      if (method %in% c('DA-QDA','all')) {
        ...
        fit <- sQDA(X_train[1:n1,], X_train[(n1+1):(n1+n2),], options)
        output <- sQDAClassify(fit, X_test)
      ...
      if (method %in% c('RPE-SN', 'all')) {
        start_rpe <- Sys.time()
        output <- RPEnsemble('StdNormal',X_train,y_train,X_test,B = 500)
      ...
      if (method %in% c('RPE-TP', 'all')) {
        start_rpe <- Sys.time()
        output <- RPEnsemble('3_point',X_train,y_train,X_test,B = 500)
